...
export default function Category() {
	...
	const columns: ProColumns<CategoryEntity>[] = [
			...
			render: (_, record) => [
				<Tooltip key="edit" title={t("common.edit")}>
					<Button
						...
						size="small"
						icon={<EditOutlined />}
						disabled={!canAccess(PermissionType.UpdateCategory)}
				...
				>
					<Tooltip title={t("common.delete")}>
						<Button
							...
							danger
							icon={<DeleteOutlined />}
							disabled={!canAccess(PermissionType.DeleteCategory)}
						/>
					</Tooltip>
				</Popconfirm>,
			],
	...
	return (
		<BasicContent className="h-full">
			<BasicTable<CategoryEntity>
				columns={columns}
				actionRef={actionRef}
				request={async (params) => {
					const { data, total } = await categoryService.fetchCategoryList(params);
					return { data, total, success: true };
				}}
				headerTitle={t("common.menu.category")}
				toolBarRender={() => [
					<Button
						key="add-category"
						icon={<PlusCircleOutlined />}
						type="primary"
						disabled={!canAccess(PermissionType.CreateCategory)}
						onClick={handleAdd}
					>
						{t("common.add")}
					</Button>,
